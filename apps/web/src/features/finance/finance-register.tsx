...
> = {
  charges: {
    activeItem: 'finance:charges',
    eyebrow: 'Finance',
    title: 'Charges',
    description: 'Open and settled tenant charges generated by billing before invoicing and allocation.',
    permission: 'billing.read',
    endpoint: '/charges',
    empty: 'No Charges match the current filters.',
    createHref: '/finance/invoices/new',
    createLabel: 'Create Invoice',
    createPermission: 'invoice.manage',
    statuses: ['OPEN', 'PARTIALLY_PAID', 'PAID', 'WRITTEN_OFF', 'CANCELLED'],
    columns: [
      { label: 'Charge', value: (row) => row.chargeNumber as string },
      { label: 'Debtor', value: (row) => text(nested(row, 'debtor', 'displayName')) },
      {
        label: 'Type',
        value: (row) => text(nested(row, 'chargeType', 'name')) || humanize(text(nested(row, 'chargeType', 'code'))),
      },
      { label: 'Due Date', value: (row) => date(row.dueDate) },
      { label: 'Outstanding', value: (row) => money(row.currency, row.outstandingAmount) },
    ],
  },
  invoices: {
    ...
    createLabel: 'Create Invoice',
    createPermission: 'invoice.manage',
    detailPath: (id) => `/finance/invoices/${id}`,
  ...
  'owner-payouts': {
    ...
    createHref: '/finance/owner-payouts/new',
    createLabel: 'Prepare Payout',
    createPermission: 'payout.manage',
    detailPath: (id) => `/finance/owner-payouts/${id}`,
    statuses: [
      'DRAFT',
      'REVIEW',
      'APPROVED',
  ...
  expenses: {
    ...
    createPermission: 'expense.manage',
    detailPath: (id) => `/finance/expenses/${id}`,
    statuses: [
      'DRAFT',
      'SUBMITTED',
      'REVIEW',
  ...
  accounting: {
    ...
    createHref: '/finance/accounting/new',
    createLabel: 'Create Journal',
    createPermission: 'journal.manage',
    detailPath: (id) => `/finance/accounting/${id}`,
                        ...
                        <DataTableRow key={row.id}>
                          {definition.columns.map((column, index) => (
                            <DataTableCell key={column.label}>
                              {definition.detailPath && index === 0 ? (
                                <Link className="text-emerald-700" href={definition.detailPath(row.id)}>
                                  {column.value(row) ?? '—'}
                                </Link>
  ...
  return (
    <div className="mb-6">
      <FinanceFormPanel
        ...
        description="Creates an immutable snapshot for the selected owner and period."
        submitLabel="Generate statement"
        busy={generate.isPending}
        disabled={!owner?.id || !periodStart || !periodEnd}
        onSubmit={() => {
          if (!owner?.id || !periodStart || !periodEnd) {
            toast.error('Choose an owner and statement period.');
            return;
          }
          generate.mutate();
        }}
