...
function CreateDialog({ mode, onClose, onSuccess }: { mode: OperationsMode; onClose: () => void; onSuccess: () => void }) {
  ...
  return <Modal title={createLabels[mode]} onClose={onClose}><form className="grid gap-4" onSubmit={(event) => { event.preventDefault(); mutation.mutate(); }}>
    ...
    {mode === 'applications' ? <><RecordPicker label="Rent lead" path="/crm/leads?intent=RENT" value={first?.id ?? ''} map={pickerMap('lead')} onChange={setFirst} /><RecordPicker label="Published rental listing" path="/rental-listings?status=PUBLISHED" value={second?.id ?? ''} map={pickerMap('listing')} onChange={setSecond} /></> : null}
    ...
    {mode === 'leases' ? <><RecordPicker label="Tenant" path="/tenants" value={second?.id ?? ''} map={pickerMap('tenant')} onChange={setSecond} /><RecordPicker label="Landlord" path="/owners" value={third?.id ?? ''} map={pickerMap('owner')} onChange={setThird} /></> : null}
    ...
    {error ? <div className="feedback feedback-error" role="alert">{error}</div> : null}<div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={mutation.isPending}>{mutation.isPending ? 'Saving…' : 'Save'}</button></div>
  </form></Modal>;
}
